...
const en: Dict = {
	...
	'papers.section.synthesisSubtitle': 'Empirical body, philosophical reading, and architectural-particle argument',
	'papers.section.synthesisNote': 'Three consolidation papers: the empirical body (DAEDALUS Review, May 2026), the philosophical companion (Configurable Universe, May 2026), and the structural argument that the electron is the framework\'s architectural particle.',
...
const zhCN: Dict = {
	...
	'papers.section.synthesisSubtitle': '经验主体、哲学诠释、架构性粒子论证',
	'papers.section.synthesisNote': '三篇整合论文:经验主体(DAEDALUS 综述,2026 年 5 月)、哲学伴篇(可配置宇宙,2026 年 5 月),以及电子作为框架架构性粒子的结构性论证。',
	...
	'papers.section.foundationsNote': 'TCG 框架架构论文(构造参考、预测清单、电弱边界公设更新),以及与 TCG 弧相邻的三条独立线索:(i) 单光子事件的边界定义零联络诠释,配以保持无信号传递的可预测性扩展(论文 17–18);(ii) 从 TCG 腔室数据到 Pati–Salam (B−L)/2 子代数的壁删除桥梁(论文 19);(iii) 量子时空谱维数流的现象学跑动分数 Laplacian 模型(论文 20)。相邻工作线在 Zenodo 上有独立记录,独立于 17 篇 TCG 论文系列。',
...
const zhTW: Dict = {
	...
	'papers.section.synthesisSubtitle': '經驗主體、哲學詮釋、架構性粒子論證',
	'papers.section.synthesisNote': '三篇整合論文:經驗主體(DAEDALUS 綜述,2026 年 5 月)、哲學伴篇(可配置宇宙,2026 年 5 月),以及電子作為框架架構性粒子的結構性論證。',
	...
	'papers.section.foundationsNote': 'TCG 框架架構論文(構造參考、預測清單、電弱邊界公設更新),以及與 TCG 弧相鄰的三條獨立線索:(i) 單光子事件的邊界定義零聯絡詮釋,配以保持無信號傳遞的可預測性擴展(論文 17–18);(ii) 從 TCG 腔室數據到 Pati–Salam (B−L)/2 子代數的壁刪除橋樑(論文 19);(iii) 量子時空譜維數流的現象學跑動分數 Laplacian 模型(論文 20)。相鄰工作線在 Zenodo 上有獨立記錄,獨立於 17 篇 TCG 論文系列。',
