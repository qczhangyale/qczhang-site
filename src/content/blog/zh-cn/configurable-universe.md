...
框架还有一条不可行定理 (弱角不可能是壳上角,60σ 排除)、三条结构约束 ($\Lambda$ 关系中的"最轻带电轻子选择规则"、弱区无量纲规范-动力学规范化目标 P5': $g_{2,W}^2 = 4/(3\pi)$、$\alpha$ 关系的剩余必须为 $\Delta_\alpha = -3.046 \times 10^{-4}$),以及一项防止事后拟合的 $\mathcal{I}[f]$ 防护机制 — 这些细节都在[预测与不可行性结论](/zh-cn/papers/04-predictions-no-go/)中。
