...
In 1908 the Hungarian physicist Loránd Eötvös set up two small masses on a torsion balance — one of platinum, one of ordinary copper — and looked for a tiny twist that would have indicated the two materials fall toward Earth at slightly different rates. He didn't find one. His measurement, accurate to a few parts per billion, became the most stringent test in its era of what we now call the *equivalence principle*: the assumption, lying at the heart of Einstein's general relativity, that gravity treats all matter the same.

Eötvös's experiment was the start of a long tradition. The "fifth force" — a hypothetical extra interaction that would couple to ordinary matter beyond the four known forces (gravity, electromagnetism, weak, strong) — has been hunted on tabletops ever since. The instruments have grown from torsion balances to atomic interferometers, optomechanical resonators, levitated nanospheres in optical traps. The bounds have grown by many orders of magnitude. And the answer, so far, has always been the same: no fifth force at present sensitivity.

Most theoretical frameworks that claim to address fundamental questions — the structure of spacetime, the origin of mass, the values of the dimensionless constants — operate at energies far beyond what tabletop experiments can probe. They are tested, when they are tested at all, only in the indirect sense of being consistent with known physics, never by forward prediction. This is not a failure of imagination; it is the geometry of the problem. The deeper one reaches, the harder it gets to find a quantity that is both calculable from the framework and measurable on a benchtop.

[Twistor Configuration Geometry (TCG)](/papers/03-tcg-fpa-model/) — the framework that proposes the dimensionless constants of nature are structural invariants of a combinatorial geometry on Penrose's projective twistor space — is unusual among ambitious theoretical programs in having one specific, falsifiable, *tabletop-accessible* prediction. The prediction is for a fifth force, of the kind Eötvös and his successors have been chasing for over a century. The framework predicts the force exists, predicts where it sits in the parameter space, and predicts that current optomechanical experiments are within roughly a factor of 500 of detecting it. The bounds are tightening at about an order of magnitude per decade. The next two to three iterations should resolve the question.
