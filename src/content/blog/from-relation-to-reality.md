...
The symbols $a$ and $b$ are distinguishable roles inside the completed relation $\mathcal C$. They are not assumed to be two fully formed objects that existed first and were connected later. The proposal is therefore one of **difference without separability**: distinction is real, but the roles receive their meaning within a larger completion.
...
| Strand | Central question | Main contribution |
|---|---|---|
| Relational foundation | What does the primitive relation mean, and what does it *not* determine? | A precise primitive, selector discipline, and independence models showing that probability, geometry, dynamics, and complex quantum theory are not already hidden in the notation. |
| Quantum reconstruction | Which operational assumptions are needed before complex quantum theory can be recovered? | A conditional reconstruction audit with exact countermodels separating the roles of local equivalence, purification, composition, and probability assumptions. |
| Two-rebit readout | Can globally real information be invisible to separated local measurements? | An exact hidden-coordinate theorem and concrete joint-gate readout procedures. |
| Operational objectivity | Can observers agree on a complete public record without determining the full state? | A finite countermodel separating reproducible public objectivity from tomographic completeness. |
| Relational time | Does a readable clock automatically give time an arrow? | A resource account of records, maintenance, fuel, waste, recurrence, and entropy export. |
| Finite/local gravity | Can Lorentzian and Regge-like structure arise from paired incidence and transport? | A conditional route from incidence to Lorentzian geometry, transport, Palatini–Regge structure, and two null polarizations on a finite periodic lattice. |
| Local Regge transfer | Why did a broad transfer rule fail, and what information was missing? | Descriptor-local source sufficiency, prospective blind tests, and a conditional curved-Regge identity. |
| Six-sector cancellation | When must a local response vanish exactly? | A Reynolds-averaging theorem for an intrinsic six-sector zero mechanism. |
| Unified Dynamics | Can quantum and gravity descendants share one finite relational architecture? | A common dual-pair/BF-type kernel, exact bridge integration, and a clearly identified variation-space obstruction. |
...
Copy depth and reference structure add another layer. Two separated copies can reveal $|s|$, while the sign remains tied to an oriented reference under the stated restrictions. The lesson is broader than rebits:
...
A separately admitted global measurement then distinguishes the two states strongly: in the paper's finite example, the public product outcomes are identical while the global query separates the states with total-variation distance $3/5$, giving an equal-prior discrimination success probability of $4/5$. The example therefore separates two achievements:

- **Operational objectivity:** qualified observers can establish a stable, reproducible fact relative to a declared interface.
- **Tomographic exhaustiveness:** that interface uniquely determines the full underlying state.
...
A separate gravity paper studies the opposite phenomenon: when symmetry forces a response to be exactly zero. For a complete six-sector orbit with exact $S_3$ equivariance, Reynolds averaging isolates the invariant part. If that part is absent, the six contributions cancel exactly.
...
- Can controlled composition and “Three-to-the-many” be derived from the primitive relation rather than merely inspired by it?
- What selects the Born rule, purification, and nonseparable composition?
- What chooses the physically relevant carrier, real structure, dimension, orientation, and absolute scale?
- Can local transport and Regge results be extended to global gluing, arbitrary meshes, and a continuum limit?
- What selects the variation space that separates gravity from other descendants?
- Can the program produce a quantitative, parameter-free experimental prediction?
...
After a finite Euclidean Jordan framework and a causal-carrier bridge are supplied, the Time branch asks what happens when a primitive role and its full algebraic complement are both required to remain primitive. Under that stronger condition, the carrier has rank two. Its nonclassical simple descendants are spin factors with one distinguished completion direction and an $m$-dimensional distinction sector. Twisting a positive pairing by the canonical complement gives signature $(1,m)$.

Within that declared framework, there is one timelike direction and $m$ spacelike directions. The value of $m$ and the absolute duration of one second are not yet fixed.
...
- $m$ planes containing the time direction, and
- $m(m-1)/2$ purely spatial planes.
...
Even then, Maxwell theory is not complete. A pointwise algebraic operator does not supply the exterior derivative, a potential $A$, the relation $F=dA$, locality, gauge redundancy, sources, or an action.
...
- Hodge action on field strengths;
- electric–magnetic duality, which rotates nonzero fields;
- gauge transformation, which changes the potential while leaving the field strength fixed.

A principal-bundle connection can be described as a horizontal/vertical complement, and curvature measures its nonintegrability. But complementarity does not choose a preferred connection. Compact $U(1)$, integral flux, charge units, monopoles, and the observed matter spectrum all require further global or material input.
...
Release II also separates Hodge chirality $\sigma$, frequency sign $\operatorname{sgn}(\omega)$, and helicity $h$. With fixed conventions,
...
$$
\text{completed relation}
\longrightarrow
\text{selected carrier}
\longrightarrow
\text{composition and geometry}
\longrightarrow
\text{differential dynamics}
\longrightarrow
\text{modes}
\longrightarrow
\text{quantization}
\longrightarrow
\text{matter and measurement}.
$$
...
- Can controlled composition be derived from the primitive completed relation?
- Can the “projection of a projection” idea become a genuine account of objective probability and the Born rule?
- What selects purification, complex amplitudes, and nonseparable composition?
- Why should the physical field degree be two, and what selects the sector exchange that yields three spatial dimensions?
- Can a unique constitutive or Hodge structure be selected rather than supplied?
- Can finite/local gravity survive arbitrary meshes, global gluing, refinement, and a continuum limit?
- What selects a positive-frequency structure and vacuum on general backgrounds?
- What chooses the variation space, matter content, charge pattern, and absolute physical scales?
- Can the program produce a parameter-free prediction or a realizable multiclock test?
...
3. [Relational Time without a Free Arrow: Record Capacity, Negentropy Transport, and Finite Autonomous Maintenance](https://zenodo.org/records/21926033) — Separates relational clock order from a thermodynamic arrow through records, memory, repair, fuel, waste, and recurrence.
4. [Certificate-Relative Operational Objectivity without Tomographic Exhaustiveness: Public Interfaces, Evidence Pedigrees, and a Two-Rebit Countermodel](https://zenodo.org/records/21926143) — Defines operational objectivity through public procedures and evidence certificates without assuming that public measurements reveal the complete state.
5. [Complementarity Before Quantum Theory: Assumption Control, Countermodels, and Conditional Complex Closure](https://zenodo.org/records/21926050) — Audits the extra assumptions required to move from primitive complementarity toward operational and complex quantum theory, with exact countermodels.
6. [Complementarity-First TCG: Paired Incidence, Transport, and the Finite/Local Gravity Architecture](https://zenodo.org/records/21926054) — Builds the parent paired-incidence and transport architecture for conditional finite/local Lorentzian and Regge gravity.
7. [Local H2/H2R Source Sufficiency, Blind Regge Transfer, and Conditional Curved-Regge Identity](https://zenodo.org/records/21926066) — Studies when local H2/H2R data are sufficient for blind Regge transfer and a conditional curved-Regge identity.
8. [An Intrinsic Six-Sector Reynolds Cancellation Theorem for Local Regge Zero Germs](https://zenodo.org/records/21926078) — Proves an intrinsic six-sector Reynolds cancellation mechanism for local Regge zero germs and validates it on finite cases.
...
3. [Quantum Clocks in Complementarity-First Geometry: Redshift, Equivalence, and the Universal Matter-Coupling Boundary](https://zenodo.org/records/22072876) — Separates relational interval, clock phase, redshift, and matter coupling, and identifies a three-level spectral-shape test boundary.
4. [Complementarity Before Electromagnetism: Paired Kinematics, One-Time Geometry, and Conditional Four-Dimensional Hodge Structure](https://zenodo.org/records/22072884) — Derives a conditional one-time-plus-three-space balance for two-form sectors and lists the extra assumptions needed for Hodge structure.
5. [Conditional Maxwell Reconstruction: Differential Closure, Gauge Geometry, Global Charge, and an Exact Abelian Descendant](https://zenodo.org/records/22072886) — Reconstructs Maxwell theory step by step from supplied differential, gauge, action, and global data, including an exact Abelian descendant.
